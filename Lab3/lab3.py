# coding: cp1251
import argparse                     # Для чтения аргументов (--input, --output, --pixel-size)
import os                           # Работа с путями
import cv2                          # OpenCV — читаем и сохраняем изображения
import numpy as np                  # NumPy — работа с массивами
import matplotlib.pyplot as plt     # Matplotlib — визуализация до/после
...
def pixelate_image(image: np.ndarray, pixel_size: int) -> np.ndarray:
    ...
    """
    if pixel_size <= 1:
        return image.copy()  # если размер 1 — ничего не меняем

    h, w = image.shape[:2]

    # 1. Уменьшаем изображение в pixel_size раз
    small = cv2.resize(image,
                       (w // pixel_size, h // pixel_size),   # новый размер
                       interpolation=cv2.INTER_LINEAR)       # обычное сжатие

    # 2. Увеличиваем обратно до исходного размера методом "ближайшего соседа"
    #    Это и даёт эффект больших квадратных пикселей
    pixelated = cv2.resize(small,
                           (w, h),                               # обратно до исходного размера
                           interpolation=cv2.INTER_NEAREST)      # ? ВАЖНО: ближайший сосед = пиксель-арт
